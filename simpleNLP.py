...
######################################################################
 ...
 def examine_text(self) :
     ...
     regex = re.compile(re.escape(start_phrase)+'\s+(\w.*)\Z',re.I)
     match = re.search(regex,str(text))
     if start_phrase and match : text = str(match.group(1))
       ...
       for negation_index in range(n_scope[0],n_scope[1]+1) :
        for t_p in self.target_phrases :
         ...
         if re.search(regex,words[negation_index]) :
          ...
          negated = 1
          break
        # i think this is not needed so we can see negex for all target_phrases
        #if negated > 0 : break
